...
$(function () {
  ...
  const curPitchEl = $("#cur-pitch");
  const curFreqEl = $("#cur-freq");
  const curVolEl = $("#cur-vol");
  const curDurEl = $("#cur-dur");
  const sliderP = $("<div id='slider-p'></div>");
  ...
  // fn to display note in HTML
  const dispNote = (ddNoteVal, freqName, dbLv) => {
    curPitchEl.text("Pitch: " + ddNoteVal);
    curFreqEl.text("Frequency: " + freqName[0].frequency + " Hz");
    curVolEl.text("Volume: " + curVolVal + " db");
    var curDurNota = Tone.Time(curDurVal).toNotation();
    curDurEl.text("Duration: " + curDurVal * 1000 + " ms, " + curDurNota);
  };

  // Get matching frequency object
  const matchNote = (noteVal) => {
